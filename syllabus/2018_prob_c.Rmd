...
- Aulas : 4a, 10h-12h; e 6a, 16h-18h

- Instruções para o envio de e-mails: 
Por favor adicione ``[Probabilidade C]'' ao 
...
- [Notas de aula](https://www.overleaf.com/read/fjdsbxwydcnf) estão disponíveis online.
  Vocês deverão baixar, imprimir e
...
discutir comigo a entre si as listas de exercícios.
...
- **Prova 1**: Ocorrerá no dia 13/04. 
Incluirá todos os tópicos vistos em aula.

- **Prova 2**: Ocorrerá no dia 11/05. 
Incluirá todos os tópicos vistos em aula,
com ênfase no material após a Prova 1.

- **Prova 3**: Ocorrerá no dia 29/05. 
...
de uma lista de exercícios ou prova até 5 dias após a devolução desta.
Se você estiver ausente no dia de entrega da lista de exercícios, o prazo não será extendido.
Para requerer avaliação informe as questões que você acredita que foram corrigidas erroneamente
e a motivação para o seu raciocínio.
...
Em particular, é **expressamente proibido** copiar lições de casa ou
...
```{r echo = FALSE}
`%>%` = magrittr::`%>%`
calendar_table = yaml::read_yaml("calendars/2018_prob_c.yaml") %>% 
  do.call(rbind, .) %>% 
  as.data.frame() %>% 
  tidyr::unnest() %>%
  tibble::as.tibble() %>%
